/*
Harvester APIs

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1beta1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the KubevirtIoApiCoreV1DataVolumeTemplateSpec type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &KubevirtIoApiCoreV1DataVolumeTemplateSpec{}

// KubevirtIoApiCoreV1DataVolumeTemplateSpec struct for KubevirtIoApiCoreV1DataVolumeTemplateSpec
type KubevirtIoApiCoreV1DataVolumeTemplateSpec struct {
	ApiVersion string `json:"apiVersion"`
	Kind string `json:"kind"`
	Metadata *K8sIoV1ObjectMeta `json:"metadata,omitempty"`
	Spec KubevirtIoContainerizedDataImporterApiPkgApisCoreV1beta1DataVolumeSpec `json:"spec"`
	Status map[string]interface{} `json:"status,omitempty"`
}

type _KubevirtIoApiCoreV1DataVolumeTemplateSpec KubevirtIoApiCoreV1DataVolumeTemplateSpec

// NewKubevirtIoApiCoreV1DataVolumeTemplateSpec instantiates a new KubevirtIoApiCoreV1DataVolumeTemplateSpec object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewKubevirtIoApiCoreV1DataVolumeTemplateSpec(apiVersion string, kind string, spec KubevirtIoContainerizedDataImporterApiPkgApisCoreV1beta1DataVolumeSpec) *KubevirtIoApiCoreV1DataVolumeTemplateSpec {
	this := KubevirtIoApiCoreV1DataVolumeTemplateSpec{}
	this.ApiVersion = apiVersion
	this.Kind = kind
	this.Spec = spec
	return &this
}

// NewKubevirtIoApiCoreV1DataVolumeTemplateSpecWithDefaults instantiates a new KubevirtIoApiCoreV1DataVolumeTemplateSpec object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewKubevirtIoApiCoreV1DataVolumeTemplateSpecWithDefaults() *KubevirtIoApiCoreV1DataVolumeTemplateSpec {
	this := KubevirtIoApiCoreV1DataVolumeTemplateSpec{}
	return &this
}

// GetApiVersion returns the ApiVersion field value
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetApiVersion() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ApiVersion
}

// GetApiVersionOk returns a tuple with the ApiVersion field value
// and a boolean to check if the value has been set.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetApiVersionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ApiVersion, true
}

// SetApiVersion sets field value
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) SetApiVersion(v string) {
	o.ApiVersion = v
}

// GetKind returns the Kind field value
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetKind() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Kind
}

// GetKindOk returns a tuple with the Kind field value
// and a boolean to check if the value has been set.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetKindOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Kind, true
}

// SetKind sets field value
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) SetKind(v string) {
	o.Kind = v
}

// GetMetadata returns the Metadata field value if set, zero value otherwise.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetMetadata() K8sIoV1ObjectMeta {
	if o == nil || IsNil(o.Metadata) {
		var ret K8sIoV1ObjectMeta
		return ret
	}
	return *o.Metadata
}

// GetMetadataOk returns a tuple with the Metadata field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetMetadataOk() (*K8sIoV1ObjectMeta, bool) {
	if o == nil || IsNil(o.Metadata) {
		return nil, false
	}
	return o.Metadata, true
}

// HasMetadata returns a boolean if a field has been set.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) HasMetadata() bool {
	if o != nil && !IsNil(o.Metadata) {
		return true
	}

	return false
}

// SetMetadata gets a reference to the given K8sIoV1ObjectMeta and assigns it to the Metadata field.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) SetMetadata(v K8sIoV1ObjectMeta) {
	o.Metadata = &v
}

// GetSpec returns the Spec field value
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetSpec() KubevirtIoContainerizedDataImporterApiPkgApisCoreV1beta1DataVolumeSpec {
	if o == nil {
		var ret KubevirtIoContainerizedDataImporterApiPkgApisCoreV1beta1DataVolumeSpec
		return ret
	}

	return o.Spec
}

// GetSpecOk returns a tuple with the Spec field value
// and a boolean to check if the value has been set.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetSpecOk() (*KubevirtIoContainerizedDataImporterApiPkgApisCoreV1beta1DataVolumeSpec, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Spec, true
}

// SetSpec sets field value
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) SetSpec(v KubevirtIoContainerizedDataImporterApiPkgApisCoreV1beta1DataVolumeSpec) {
	o.Spec = v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetStatus() map[string]interface{} {
	if o == nil || IsNil(o.Status) {
		var ret map[string]interface{}
		return ret
	}
	return o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) GetStatusOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Status) {
		return map[string]interface{}{}, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given map[string]interface{} and assigns it to the Status field.
func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) SetStatus(v map[string]interface{}) {
	o.Status = v
}

func (o KubevirtIoApiCoreV1DataVolumeTemplateSpec) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o KubevirtIoApiCoreV1DataVolumeTemplateSpec) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["apiVersion"] = o.ApiVersion
	toSerialize["kind"] = o.Kind
	if !IsNil(o.Metadata) {
		toSerialize["metadata"] = o.Metadata
	}
	toSerialize["spec"] = o.Spec
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	return toSerialize, nil
}

func (o *KubevirtIoApiCoreV1DataVolumeTemplateSpec) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"apiVersion",
		"kind",
		"spec",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varKubevirtIoApiCoreV1DataVolumeTemplateSpec := _KubevirtIoApiCoreV1DataVolumeTemplateSpec{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varKubevirtIoApiCoreV1DataVolumeTemplateSpec)

	if err != nil {
		return err
	}

	*o = KubevirtIoApiCoreV1DataVolumeTemplateSpec(varKubevirtIoApiCoreV1DataVolumeTemplateSpec)

	return err
}

type NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec struct {
	value *KubevirtIoApiCoreV1DataVolumeTemplateSpec
	isSet bool
}

func (v NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec) Get() *KubevirtIoApiCoreV1DataVolumeTemplateSpec {
	return v.value
}

func (v *NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec) Set(val *KubevirtIoApiCoreV1DataVolumeTemplateSpec) {
	v.value = val
	v.isSet = true
}

func (v NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec) IsSet() bool {
	return v.isSet
}

func (v *NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableKubevirtIoApiCoreV1DataVolumeTemplateSpec(val *KubevirtIoApiCoreV1DataVolumeTemplateSpec) *NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec {
	return &NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec{value: val, isSet: true}
}

func (v NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableKubevirtIoApiCoreV1DataVolumeTemplateSpec) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


